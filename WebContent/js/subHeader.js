$(function() {
	...
	searchForm += `<form action="/mangoplate/board?cmd=search" method="POST" class="search-wrap w-50" >`;
	searchForm += `<i class="material-icons mt-1" style="position: absolute; left: 170px;">search</i>`;
	searchForm += `<input type="text" placeholder="지역, 식당 또는 음식" class="ml-5 mt-1 rounded w-50  " name="keyword"style="padding-left: 40px; outline: none; border: none;">`;
	searchForm += `</form>`;
  ...
  function searchEnter() {
        if ( window.event.keyCode == 13 ) {
			window.location.href='http://localhost:8000/mangoplate/board?cmd=search';
        }
    }